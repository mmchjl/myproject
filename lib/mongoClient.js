...
module.exports.findList = function(opt,callback){
    var collection = opt.collection;
    var query = opt.query||{};
    var fields = opt.fields||{};
    mongo(function(err,client,release,genid){
        if(!utility.isUndefined(client)){
            client.collection(collection,function(err,coll){
                ...
                coll.find(query,fields).toArray(function(err,data){
                    if(err){
                        release()
                        utility.handleException(err);
                        return callback(err,[]);
...
module.exports.execute=function(sp,params,callback){
    var SP = utility.Format(sp,params);
    mongo(function(err,client,release,genid){
        if(err){
            release();
            utility.handleException(err);
        ...
        //console.log(SP);
        client.eval(SP,function(err,data){
            if(err){
                release();
                utility.handleException(err);
                return callback(err);
