...
var captchapath =  '/img/captcha_pic/',
    captcha_ary = [
    ...
    mongo.mongo(function(err, db, release){
        ...
        db.collection(captcha.dbcolname, function(err, collection){
            ...
            collection.insert(ary[po], function(err, doc){
                release();
                if (err){
                    utility.handleException('创建验证码插入数据失败'+err);
                    return cb(false);
                }
    ...
    mongo.mongo(function(err, db, release,get_id){
        db.collection(captcha.dbcolname, function(err, collection){
            ...
            collection.findOne({'_id':get_id(db, id)},function(err, doc){
                release();
                if(err) return utility.handleException('对比验证码失败：'+err);
...
captcha.intial = function(){ //初始化，异步脚本删除验证码
    process.nextTick(function(){
        mongo.mongo(function(err, db, release){ //建立集合
            if (err) return release();
            db.createCollection(captcha.dbcolname, function(err, col){
                release()
                if(err) utility.handleException('创建验证码集合失败：'+err)
            })
        });
